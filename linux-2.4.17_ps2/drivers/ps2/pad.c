 ...
 *
 * $Id: pad.c,v 1.1.2.11 2002/11/20 10:48:14 takemura Exp $
 */
...
ps2pad_read(struct file *filp, char *buf, size_t size, loff_t *off)
{
	...
	// Only try to swap buttons if we have enough bytes and console region is not Japanese
	res = (data[1] & 0x0f) * 2 + 2;
	if (res >= 6 &&
	    strcmp(ps2_bootinfo->ver_dvd_id, "J") != 0 &&
	    strcmp(ps2_bootinfo->ver_dvd_id, "") != 0) {
...
ps2pad_cleanup(void)
{
	...
	if ((init_flags & INIT_DEV) &&
	    unregister_chrdev(ps2pad_major, "ps2pad") < 0) {
